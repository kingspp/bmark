...
class GPUStat(object):
    ...
    @property
    def current_graphics_clock(self):
        """
        Current frequency of graphics (shader) clock.
        :return:
        """
        v = self.entry['clocks.current.graphics']
        return v if v is not None else None

    @property
    def max_graphics_clock(self):
        """
        Maximum frequency of graphics (shader) clock.
        :return:
        """
        v = self.entry['clocks.max.graphics']
        return v if v is not None else None

    @property
    def current_sm_clock(self):
        """
    ...
    def max_sm_clock(self):
        ...
        :return:
        """
        v = self.entry['clocks.max.sm']
        return v if v is not None else None

    @property
    def current_memory_clock(self):
        """
        Current frequency of memory clock.
        :return:
        """
        v = self.entry['clocks.current.memory']
        return v if v is not None else None
    ...
    def max_memory_clock(self):
        ...
        :return:
        """
        v = self.entry['clocks.max.memory']
        return v if v is not None else None

    @property
    def power_drawn(self):
        """
...
class GPUStatCollection(object):
    ...
    def new_query():
        ...
        gpu_query_columns = ('index', 'uuid', 'name', 'memory.used', 'memory.total', 'clocks.current.graphics',
                             'clocks.max.graphics', 'clocks.current.sm', 'clocks.max.sm', 'clocks.current.memory',
                             'clocks.max.memory', 'temperature.gpu', 'utilization.gpu', 'power.draw', 'power.limit')
        gpu_list = []
...
def get_gpu_stats():
    """
    Return the GPU statistics in an Ordered dict
    """
    ret_gpu_stats = {}
    try:
        gpu_stats = GPUStatCollection.new_query()
